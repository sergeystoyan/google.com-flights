...
Developed by: www.cliversoft.com";
        ...
        public class FlightSearchItem : InputItem
        {
            ...
            readonly public string Mobile_SmsGateways;
            readonly public double AlertFactor;
            [ConstructedField]
            readonly public UserInfo[] UserInfos;
            ...
            public FlightSearchItem()
            {
                ...
                foreach (string ui in Regex.Split(Mobile_SmsGateways, @";"))
                {
                    Match m = Regex.Match(ui, @"(.*?)_(.*)");
                    uis.Add(new UserInfo() { Mobile = m.Groups[1].Value.Trim(), SmsGateway = m.Groups[2].Value.Trim() });
                }
                UserInfos = uis.ToArray();
            }
